...
impl Workspaces {
    pub fn get(self, workspace_id: Option<String>) -> Workspace {
        ...
        let req = reqwest::blocking::Client::new().get(url).header("Authorization", header).query(&("workspace_id", query)).send().expect("Failed to send get request");
        ...
        let json: Workspace = req.json().expect("Failed to deserialize JSON");
        json
    }
